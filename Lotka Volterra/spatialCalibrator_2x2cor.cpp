...
void backwardVolatility(const vec& n, const vec& d, 
  ...
  v(T-1) = 1.0/vinv;
  for(int t=T-2; t>=0; t--){
    v_star = Rcpp::rgamma(1,(1-delta_v)*n(t), 1.0/d(t))(0);
...
//////////////// //////////////// //////////////// 11/20/2023
// sample h(T) used in paper revision
// page 172 in DLM in R
void sampleT(const mat& theta, const vec& d, 
                        const double delta_v, vec& v){
                     ...
                     double& sigma2_z, const int Sp)
{
  ...
  ll_star = ll_counter_star+calPrior(calibrate_star), ll=ll_counter+calPrior(calibrate);
...
void sampleVar(mat z, mat emulated, double& sigma2, const double z_a, const double z_b){
  double SSR = accu(pow(z - emulated.t(),2));
...
Rcpp::List spDLMGP(const mat& z, const mat& y,  const cube& y_cube, const vec yinit,
                              ...
                              const double nugget, double z_a, double z_b)
{
  int T = y.n_cols;
  int p = y.n_rows;
  int S = z.n_rows;
  ...
  for(uword i=0; i<niter; i++) {
    ...
    if(i >= burnin){
      ...
      mc_C.slice(i-burnin) = C.slice(T-1);
    }
  }
