...
gasp = dlmGaSP_calibrator(z=z[,-1], y=yin[,-1],
                          yy = array(ysp2[,-1,],dim=c(nrow(ysp2),ncol(ysp2)-1,dim(ysp2)[3])),
                          FF=FF,FFc = FFsp,
                          m0=matrix(rep(0,p2)),
                          yinit = unique(yin[,1]),
                          C0=1*diag(p2), alpha0=.1, beta0=.1,
                          delta_v=.9, delta_w=.9,
                          params=theta_design,
                          pred_params = test,
                          tune=rep(.01,4), tune2=rep(.01,4), tune3=.25,
