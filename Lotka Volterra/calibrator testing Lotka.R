...
# theta_design[,1] <- qtruncnorm(A[,1], a=0,b=Inf,mean = test[1],sd=.5)
# theta_design[,2] <- qtruncnorm(A[,2], a=0,b=Inf,mean = test[2],sd=.01)
# theta_design[,3] <- qtruncnorm(A[,3], a=0,b=Inf,mean = test[3],sd=.5)
# theta_design[,4] <- qtruncnorm(A[,4], a=0,b=Inf,mean = test[4],sd=.01)
...
#           ylab="log-population", xlab="Year", xaxt="n")
#   axis(1, at = seq(1, length(times), by=10), 
#        labels = seq(1,21),
#        las=2)
...
spgasp = parallel_mvcalibrator_discrepancy(z=as.matrix(z[-1,],ncol=2), 
                                           y = array(ysp[,-1,],
                                                     dim=c(nrow(ysp),ncol(ysp)-1,dim(ysp)[3])), 
                                           niter = 5000, burnin = 5000/2,
                                           m0=matrix(rep(0,p2)), C0=diag(p2), 
                                           FF = FFsp, G=1*diag(p2),
                                           params = theta_design,
...
matplot((t(spgasp$mc_sigma2)),type='l')
...
for(i in 1:length(test)){
  ...
  if(i==1){
    ...
    legend(.24, 15, c("Prior", "Emulator/Calibrator", "Stan"), 
           fill=c(rgb(.678, .847, .902),
                  rgb(0/255,0/255,.502, .7), 
                  "red"), bty='n', text.font=2,cex=1.5)
  }else{
    hist(tempTest[,i], xlim=c(0,1), breaks=10, probability = 1, xlab=TeX(
      paste0('$\\eta_', i, "$")), main="",
      col=rgb(1, 0, 0, alpha=.5),add=0)
...
apply(t(spgasp$mc_beta), 2, matplot, type="l")
{
  ...
  for(i in 1:4){
    ...
    hist(thinCal[,i],breaks=10,xlim=c(0,1), add=0, probability = 1)
    hist(rbeta(nrow(thinCal), 1, 1), breaks=10,add=1, probability = 1,
...
median = c(apply(spgasp$mc_yeta,c(1,3),median)+(apply(spgasp$mc_w,1:2,quantile,.5)))
...
matplot(apply(spgasp$mc_yeta,c(1,3),median)+(apply(spgasp$mc_w,1:2,quantile,.5)), lty=1,lwd=4, type="l",
...
colnames(df2) = c("Measurement","Cases","Median","q5","q95","group")
{
  h1 <- ggplot(data=df2, aes(y=Median, x=Measurement,color=factor(group),fill=factor(group)))+ggtitle("Stan Fit")+
    geom_ribbon(aes(ymin=q5, ymax=q95,color=factor(group),alpha=factor(group)), linetype=0) +
    theme_classic()+coord_cartesian(ylim = c(0, 2*max(median)))+
